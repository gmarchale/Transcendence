...
async function loadPendingRequests(){
	try {
		const response = await fetch('/api/chat/get_pending_requests/', {
			method: 'GET',
			headers: { 'Content-Type': 'application/json', 'X-CSRFToken': getCookie('csrftoken') }
		});
		const data = await response.json();
		
		const pendingList = document.getElementById("friends_pending_list");
		pendingList.innerHTML = '';
		
		if(data.pending && data.pending.length > 0){
			document.getElementById("friends_pending_container").style.display = "block";
			data.pending.forEach(request => {
				const requestElement = document.createElement('div');
				requestElement.className = 'friends_item';
				requestElement.innerHTML = `
					<div class="friends_item-info">
						<img src="/images/logo.jpg" alt="User Avatar" class="friends_avatar">
						<span class="friends_username">${request.username}</span>
					</div>
					<div class="friends_item-actions">
						<button class="friends_accept-btn" onclick="manageFriendship('${request.id}', 'accept_friend_request')">Accept</button>
						<button class="friends_decline-btn" onclick="manageFriendship('${request.id}', 'delete_friend_user')">Decline</button>
					</div>
				`;
				pendingList.appendChild(requestElement);
			});
		} else {
			document.getElementById("friends_pending_container").style.display = "none";
		}
	} catch (error) {
...
function initFriends(){
	...
	document.getElementById('friends_search_button').addEventListener('click', searchUsers);
...
async function searchUsers(){
	...
	try {
		const response = await fetch('/api/chat/search_users/', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json', 'X-CSRFToken': getCookie('csrftoken') },
